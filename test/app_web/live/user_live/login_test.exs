defmodule AppWeb.UserLive.LoginTest do
  ...
  describe "login page" do
    test "renders login page", %{conn: conn} do
      ...
      assert html =~ "Welcome to Tilawah"
      assert html =~ "Create an account"
      assert has_element?(_lv, "#login_form_password")
    end
  end
  ...
  describe "login navigation" do
    test "redirects to registration page when the Create an account button is clicked", %{
      conn: conn
    } do
      {:ok, lv, _html} = live(conn, ~p"/users/log-in")
  ...
  describe "re-authentication (sudo mode)" do
    ...
    test "shows login page with email filled in", %{conn: conn, user: user} do
      ...
      assert html =~ "Please reauthenticate"
